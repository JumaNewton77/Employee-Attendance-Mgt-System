        ...
        <div class="container-fluid">
			<table class="table table-stripped table-hover">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT * from `department` order by unix_timestamp(dptdate) desc, unix_timestamp(dptdate) desc ");
						while($row = $qry->fetch_assoc()):
                            $row['dpthod'] = strip_tags(stripslashes(html_entity_decode($row['dpthod'])));
					?>
						<tr title="<?php echo $row['dpthod'] ?>">
							<td class="text-center"><?php echo $i++; ?></td>
							<td><?php echo $row['dptname'] ?></td>
							<td ><p class="truncate m-0"><?php echo $row['dpthod'] ?></p></td>
							<td><?php echo ($row['dptdate'] != null) ? date('Y-m-d H:i',strtotime($row['dptdate'])) : date('Y-m-d H:i',strtotime($row['dptdate'])); ?></td>
